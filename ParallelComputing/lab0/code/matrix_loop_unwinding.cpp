...
// 填充矩阵到最近的2的幂次方大小
vector<vector<double>> pad_matrix(const vector<vector<double>>& matrix, int new_rows, int new_cols) {
    vector<vector<double>> padded(new_rows, vector<double>(new_cols, 0.0));
...
int main() {
    ...
    // 使用Strassen算法
    vector<vector<double>> C = strassen(A, B, m, n, k);

    auto end_time = chrono::high_resolution_clock::now();
    double compute_time = chrono::duration_cast<chrono::duration<double>>(end_time - start_time).count();

    // 性能计算
    long long total_flops = pow(n, log2(7)) * 7; // 近似计算Strassen算法的FLOPS
